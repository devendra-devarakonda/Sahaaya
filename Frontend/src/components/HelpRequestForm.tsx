...
import { getCurrentUserRole, getRoleErrorMessage } from '../utils/roleValidation';
  ...
  useEffect(() => {
    const checkRole = async () => {
      try {
        ...
        if (!role) {
          setError('Please log in to create a help request.');
          return;
        }
        
        if (role.role !== 'individual') {
          setError('Only Individual Users can create help requests. NGO accounts should create campaigns instead.');
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestData = {
        ...
        urgency: formData.urgency as 'low' | 'medium' | 'critical',
        location: formData.fullLocation || `${formData.address}, ${formData.city}${formData.state ? ', ' + formData.state : ''}${formData.pincode ? ' - ' + formData.pincode : ''}`,
        contactInfo: `${formData.name} - ${formData.phone}`
      };

      await helpRequestsApi.create(requestData);
      setIsSubmitted(true);
      ...
      const errorMessage = getRoleErrorMessage(err as Error, 'submit help request');
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Badge variant="outline" className="text-sm" style={{ borderColor: '#41695e', color: '#41695e' }}>
                Request ID: HR-{Math.random().toString(36).substr(2, 9).toUpperCase()}
              </Badge>
            </div>
            <Button 
