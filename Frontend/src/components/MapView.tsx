        ...
        if ('geolocation' in navigator) {
          ...
          navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
              ...
              console.info('Geolocation error details:', {
                code: error.code,
              ...
              switch (error.code) {
                case error.PERMISSION_DENIED:
                  ...
                  console.info('User denied location access - using Mumbai as default location');
                  break;
                case error.POSITION_UNAVAILABLE:
                  errorMessage = 'Location information unavailable';
                  setLocationStatus('denied');
                  console.info('Location unavailable - using Mumbai as default location');
                  break;
                case error.TIMEOUT:
                  errorMessage = 'Location request timed out';
                  setLocationStatus('denied');
                  console.info('Location request timeout - using Mumbai as default location');
                  break;
                default:
                  errorMessage = `Location error: ${error.message || 'Unknown error'}`;
                  setLocationStatus('denied');
                  console.info('Location error - using Mumbai as default location:', error.message || 'Unknown error');
                  break;
              }
              
              // Always set Mumbai as fallback - this is not an error, it's expected behavior
    ...
    try {
      ...
      if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            console.info('Location access denied on retry - continuing with Mumbai location', {
              code: error.code,
              message: error.message,
              timestamp: new Date().toISOString()
            });
            setLocationStatus('denied');
            // Keep using Mumbai coordinates
          },
          {
        ...
        console.info('Geolocation not supported - continuing with Mumbai location');
        setLocationStatus('denied');
      }
    } catch (err) {
      console.warn('Unexpected error during location retry:', {
        error: err,
        message: err instanceof Error ? err.message : 'Unknown error',
        timestamp: new Date().toISOString()
      });
